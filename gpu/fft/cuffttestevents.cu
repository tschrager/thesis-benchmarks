...
int main ()
{
    ...
    long long nx;
    long long batch;
    cudaEvent_t complete_fft_timer_start, complete_fft_timer_stop;
    ...
    cudaEventCreate(&complete_fft_timer_start); 
    cudaEventCreate(&complete_fft_timer_stop);     
    ...
    //for(nx=4096; nx<=MAX_NX; nx+=4096)
    {
        ...
        for(batch=MIN_BATCH;batch<=MAX_BATCH;batch=batch*2)
        {
            ...
            {  
                ...
                cudaThreadSynchronize();
                // run the fft
                cudaEventRecord(complete_fft_timer_start, 0);
                cufftExecC2C(plan,gpudata,fftgpudata,CUFFT_FORWARD);
                cudaThreadSynchronize();
                cudaEventRecord(complete_fft_timer_stop, 0);
                ...
                float complete_fft_timer_time;
                cudaEventElapsedTime(&complete_fft_timer_time,complete_fft_timer_start, complete_fft_timer_stop);
                fprintf(stderr, "%lld\t%lld\t%f\t%f\t%f\t%f\t%f\t%f\t%f\n",
                    nx, batch, 
                    complete_fft_timer_time, sdkGetTimerValue(&complete_fft_timer2), sdkGetTimerValue(&piecewise_fft_timer), 
                    sdkGetTimerValue(&copy_to_gpu_timer), 
                    sdkGetTimerValue(&fft_only_timer), sdkGetTimerValue(&copy_from_gpu_timer),
                    complete_fft_timer_time/(nx*batch));
            }
        }
    }
