...
int main ()
{
    ...
    cudaMallocHost(&data, sizeof(cufftReal)*MAX_DIM);
    cudaMallocHost(&result, sizeof(cufftComplex)*(2*MAX_NX+1)*MAX_BATCH);
    ...
    fprintf(stderr, "Testing cufft R2C 1D fft with cudaMallocHost allocated memory\n");
    ...
    //for(nx=4096; nx<=MAX_NX; nx+=4096)
    {
        ...
        for(batch=MIN_BATCH;batch<=MAX_BATCH;batch=batch*2)
        {
            ...
            {  
                int inputsize = sizeof(cufftReal)*nx*batch;
                int resultssize = sizeof(cufftComplex)*(2*nx+1)*batch;
                //fprintf(stderr, "Allocating %lld bytes\n", sizeof(cufftComplex)*nx*batch*2);
                // allocate device memory for the fft
                cudaMalloc((void**)&gpudata, inputsize);
                cudaMalloc((void**)&fftgpudata,resultssize);
